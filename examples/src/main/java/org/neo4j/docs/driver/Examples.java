...
public class Examples
{
    ...
    public static void accessRecord( Session session ) throws Exception
    {
        ...
        StatementResult result = session.run( "MATCH (weapon:Weapon) WHERE weapon.owner CONTAINS {term} RETURN weapon.name AS Sword",
                Values.parameters( "term", searchTerm ) );
    ...
    public static Driver connectWithBasicAuth() throws Exception
    {
        // tag::connect-with-basic-auth[]
        Driver driver = GraphDatabase.driver( "bolt://localhost", AuthTokens.basic("username", "password"),
                Config.build()
                .withEncryptionLevel( Config.EncryptionLevel.REQUIRED )
                .withTrustStrategy( Config.TrustStrategy.trustSignedBy( new File( "/path/to/ca-certificate.pem") ) )
                .toConfig() );
        // end::connect-with-basic-auth[]
